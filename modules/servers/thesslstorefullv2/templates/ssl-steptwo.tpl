...
{if $majorStatus eq "initial"}
    <form class="sslgen" id="frmStepTwo" method="post" action="{$smarty.server.PHP_SELF}?action=productdetails&id={$serviceId}&page=complete">
        ...
        {if $authDomainsList|@count gt 1}
            <div class="form-group checkbox">
                {foreach from=$DCVAuthMethods key=dcvKey item=dcvValue}
                    <input type="radio" id="sslAuthMethodAll_{$dcvKey}" data-id="{$dcvValue}" name="sslAuthMethodAll" value="{$dcvKey}" {if "EMAIL" eq $dcvKey} checked="checked"{/if}><label for="sslAuthMethodAll_{$dcvKey}">  {$dcvKey} For All Domain</label><br/>
                {/foreach}
                {if $vendorName eq 'COMODO' || $vendorName eq 'SECTIGO'}
                    <input type="radio" id="sslAuthMethodAll_EMAIL" data-id="{$dcvValue}" name="sslAuthMethodAll" value="EMAIL"><label for="sslAuthMethodAll_EMAIL">  EMAIL For All Domain</label><br/>
                {/if}
            </div>
                    ...
                    <td>
                        <div class="form-group">
                            ...
                            <select name="sslDCVAuthMethod[{$authDomain|replace:'*':'star'}]" data-domain="{$authDomain}" class="form-control sslDCVMethod">
                                ...
                                {foreach from=$DCVAuthMethods key=dcvKey item=dcvValue}
                                    <option value="{$dcvKey}"{if (isset($certInfo.sslDCVAuthMethod.{$authDomain|replace:'*':'star'}) && $certInfo.sslDCVAuthMethod.{$authDomain|replace:'*':'star'} eq $dcvKey)} selected{/if}>
                                        {$dcvValue}
                                    </option>
                                {/foreach}
                                {if $vendorName eq 'COMODO' || $vendorName eq 'SECTIGO'}
                                    ...
                                    {if get_primary_domain($authDomain) ne $authDomain|replace:'www.':'' && get_primary_domain($authDomain) ne $authDomain|replace:'*.':''}
                                        ...
                                        {foreach from=$aliasEmails item=alias}
                                            <option value="{$alias}{$pdm}" {if (isset($certInfo.sslDCVAuthMethod.{$authDomain|replace:'*':'star'}) && $certInfo.sslDCVAuthMethod.{$authDomain|replace:'*':'star'} eq "{$alias}{$pdm}")} selected{/if}>{$alias}{$pdm}</option>
                                        {/foreach}
                                    {/if}
                                {/if}
                            </select>
        ...
        <p>
            ...
            <input id="btnsteptwo" type="submit" value="{$LANG.ordercontinuebutton}" class="btn btn-primary" />
        </p>
    </form>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('.sslDCVMethod').val($('[name="sslAuthMethodAll"]:checked').val());
            $('.sslDCVMethod').attr('id',$('[name="sslAuthMethodAll"]:checked').val());
