...
{/if}
    ...
    <form class="sslgen" id="frmStepOne" method="post" action="{$smarty.server.PHP_SELF}?action=productdetails&id={$serviceId}&page=sitelock-complete">
        ...
        <div class="form-group">
            <label for="sslEmail">{$LANG.sslEmail}:*</label>
            <input type="text" class="form-control" name="Email" id="sslEmail" value=""/>
        </div>
        <div class="form-group">
            <label for="sslDomainName">{$LANG.sslDomainName}:*</label>
            <input type="text" class="form-control" name="Domain" id="sslDomainName" value=""/>
        </div>
        {*
        <div class="form-group">
            <label for="ValidityPeriod">Validity Period:*</label>
            <select name="ValidityPeriod" id="ValidityPeriod" class="form-control">
               {foreach from=$certInfo[0].PricingInfo key=ValidityPeriod item=period}
                    <option value="{$period.NumberOfMonths}">
