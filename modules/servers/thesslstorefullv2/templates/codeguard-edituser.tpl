...
{/if}
{if $remoteid}
    <form class="sslgen" id="frmStepOne" method="post" action="{$smarty.server.PHP_SELF}?action=productdetails&id={$serviceId}&page=codeguard-complete">
        <input type="hidden" name="sslid" value="{$sslOrderId}">
        <h3>{$LANG.codeGuardUserInfoTitle}</h3>
        <div class="form-group">
            <label for="userName">{$LANG.codeGuardUserName}:*</label>
            <input type="text" class="form-control" name="userName" id="userName" value="{$userName}"/>
        </div>
        <div class="form-group">
            <label for="userEmail">{$LANG.codeGuardUserEmail}:*</label>
            <input type="text" class="form-control" name="userEmail" id="userEmail" value="{$userEmail}"/>
        </div>
